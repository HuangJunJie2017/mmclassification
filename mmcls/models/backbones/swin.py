...
from ..utils import PatchEmbedv1 as PatchEmbed
...
class ShiftWindowMSA(BaseModule):
    ...
    def forward(self, query, hw_shape, token_mask):
        B, L, C = query.shape
        H, W = hw_shape
        assert L == H * W, 'input feature has wrong size'
        ...
        if token_mask is not None:
            token_mask = token_mask.view(1, H, W, 1)
            if self.shift_size>0:
                token_mask = torch.roll(
                    token_mask,
                    shifts=(-self.shift_size, -self.shift_size),
                    dims=(1, 2))
            token_mask = self.window_partition(token_mask)
            token_mask = token_mask.view(-1, self.window_size * self.window_size, 1)
            P = token_mask.shape[0]
            token_mask = token_mask.reshape(P, 1, self.window_size * self.window_size)
            token_mask_new = token_mask * token_mask.transpose(
                1, 2) + (1 - token_mask) * (1 - token_mask).transpose(1, 2)
            token_mask_new = 1 - token_mask_new
            token_mask_new = -10000 * token_mask_new
            if attn_mask is None:
                attn_mask = token_mask_new
            else:
                attn_mask = attn_mask + token_mask_new
...
class SwinBlock(BaseModule):
    ...
    def forward(self, x, hw_shape, token_mask):
        identity = x
        x = self.norm1(x)
        x = self.attn(x, hw_shape, token_mask)
...
class SwinBlockSequence(BaseModule):
    ...
    def forward(self, x, hw_shape, token_mask):
        for block in self.blocks:
            if self.with_cp:
                x = checkpoint.checkpoint(block, x, hw_shape, token_mask)
            else:
                x = block(x, hw_shape, token_mask)
...
class MixMIMSwinTransformerCLS(BaseBackbone):
    """ Swin Transformer
    ...
    def __init__(self,
        ...
        super(MixMIMSwinTransformerCLS, self).__init__()
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            x, hw_shape, out, out_hw_shape = stage(x, hw_shape, None)
            if i in self.out_indices:
                ...
                outs.append(None)
        out = self.avgpool(outs[0].transpose(1,2))
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(MixMIMSwinTransformerCLS, self).train(mode)
        self._freeze_stages()
